...
import { useDeleteQuestion, usePublishExam } from '@/hooks/useExams'
import { toast } from 'sonner'
import { useRouter } from 'next/navigation'

const QuestionSet = () => {
  const { questions, examId, removeQuestion, setStep, resetStore } = useTestCreationStore();
    ...
    <div className="max-w-5xl mx-auto space-y-8">
            ...
            <div key={q.id} className="bg-white p-8 rounded-[32px] border border-gray-100 shadow-sm space-y-6">
              ...
              <div className="flex justify-between items-start">
                ...
                <div className="flex gap-1">
                  ...
                  <button onClick={() => handleDelete(q.id)} className="p-2.5 text-slate-300 hover:text-red-500 transition-all">
      ...
      <div className="flex justify-between items-center pt-10 border-t border-gray-100">
        ...
        <button 
          ...
          className="px-14 py-4 bg-[#8b5cf6] text-white font-bold rounded-2xl shadow-xl shadow-purple-100 hover:bg-[#7c3aed] disabled:opacity-40 transition-all"
          onClick={handlePublish}
        >
          {isPublishing ? 'Publishing...' : 'Finish & Publish Test'}
        </button>
      </div>
